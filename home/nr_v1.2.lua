-- [ Reactor Control v1.2 by P1KaChU337 ] -- Исправленная и улучшенная версия
...
-- Проверка GPU и экрана
if not component.isAvailable("gpu") then
    print("❌ Ошибка: GPU не найден!")
    print("Подключите GPU к компьютеру.")
    os.sleep(3)
    return
end

if not component.isAvailable("screen") then
    print("❌ Ошибка: Экран не найден!")
    print("Подключите экран к GPU.")
    os.sleep(3)
    return
end
...
if not success_buffer then
    print("❌ Ошибка: не найден doubleBuffering.lua")
    print("Установите OpenOS полностью.")
    os.sleep(3)
    return
end

if not success_image then
    print("⚠ Внимание: image.lua не найден (GUI может не работать)")
end
...
buffer.drawText(1, 1, 0xffffff, "Загрузка Reactor Control v1.2...")
buffer.drawChanges()
...
local function handleChatCommand(nick, cmd, args)
    ...
    if cmd == "@help" then
        ...
        say("§a@exit - выйти")
        say("§b@coolant - уровень хладагента в ME")
